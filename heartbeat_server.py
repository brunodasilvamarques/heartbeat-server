    ...
    if kiosk_id:
        ...
        try:
            ...
            master_data[today][kiosk_id] = {
                "kiosk_name": kiosk_name,
                "general_users": data.get("today_general_count", 0),
        ...
        for kiosk_id, data in sorted(group):
            ...
            for date_entry, kiosks_on_date in master_data.items():
                if kiosk_id in kiosks_on_date:
                    total_general += kiosks_on_date[kiosk_id]["general_users"]
                    days_recorded += 1

            average_general = total_general / days_recorded if days_recorded > 0 else 0
            average_general = round(average_general)
...
def download_csv():
    ...
    try:
        ...
        with open(temp_csv, "w", newline="") as csvfile:
            fieldnames = ["Date", "Kiosk Code", "Kiosk Name", "Country", "General Users", "Restricted Users"]
            ...
            for date_entry, kiosks_on_date in master_data.items():
                for kiosk_id, info in kiosks_on_date.items():
                    writer.writerow({
                        ...
                        "General Users": info.get("general_users", 0),
