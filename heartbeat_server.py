...
def heartbeat():
    ...
    if kiosk_id:
        ...
        # ✅ Save the kiosk’s full master_data.json
        incoming_master = data.get("full_master_data")
        if incoming_master:
            kiosk_master_file = os.path.join(DATA_DIR, f"{kiosk_id}_master_data.json")
            with open(kiosk_master_file, "w") as f:
                json.dump(incoming_master, f, indent=4)
            print(f"✅ Saved updated master_data.json for kiosk {kiosk_id}")
...
def dashboard():
    ...
    for country, group in sorted(grouped.items()):
        for kiosk_id, data in sorted(group):
            ...
            days_recorded = 0
            kiosk_master_file = os.path.join(DATA_DIR, f"{kiosk_id}_master_data.json")
            if os.path.exists(kiosk_master_file):
                ...
                for date_key, date_data in kiosk_master.items():
                    if date_key in ["kiosk_code", "kiosk_name", "country"]:
                        continue
                    daily_general = len(date_data.get("general_users", []))
                    total_general += daily_general
                    days_recorded += 1
...
def download_combined_csv():
    ...
    for kiosk_file in glob.glob(os.path.join(DATA_DIR, "*_master_data.json")):
        ...
        for date_key, date_data in kiosk_data.items():
            ...
            # General Users
            general_count = len(date_data.get("general_users", []))
            all_rows.append([date_key, country, kiosk_code, kiosk_name, "General Users", "-", "-", "-", general_count])
            ...
            # Footfall Left
            foot_left = len(date_data.get("footfall", {}).get("left", []))
            all_rows.append([date_key, country, kiosk_code, kiosk_name, "Footfall Left", "-", "-", "-", foot_left])

            # Footfall Right
            foot_right = len(date_data.get("footfall", {}).get("right", []))
            all_rows.append([date_key, country, kiosk_code, kiosk_name, "Footfall Right", "-", "-", "-", foot_right])
...
def email_combined_csv(file_path):
    ...
    email_data = {
        "message": {
            "subject": "📊 Combined Kiosk Master Data",
