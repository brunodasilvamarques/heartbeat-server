...
def download_csv_timestamps():
    ...
    for file in sorted(glob.glob("data/daily_user_count_timestamps_*.json")):
        date_key = os.path.basename(file).replace("daily_user_count_timestamps_", "").replace(".json", "")
        with open(file, "r") as f:
            data = json.load(f)
            kiosks_on_date = data.get(date_key, {})
            for type_name in ["general", "restricted", "left", "right"]:
                for entry in kiosks_on_date.get(type_name, []):
