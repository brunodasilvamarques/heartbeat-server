...
def download_csv():
    ...
    pattern = os.path.join(DATA_DIR, "daily_footfall_counts_*.json")
                ...
                with open(file, "r") as f:
                    ...
                    for kiosk_id, info in kiosks.items():
                        writer.writerow({
                            ...
                            "Footfall": info.get("footfall", {}).get("total", 0) if isinstance(info.get("footfall"), dict) else info.get("footfall", 0),
        ...
        with open(output_csv, "w", newline="") as csvfile:
            ...
            for file in files:
                ...
                with open(file, "r") as f:
                    data = json.load(f)
                    kiosks = data.get(date_str, {})
                    for kiosk_id, entry in kiosks.items():
                        ...
                        kiosk_name = entry.get("kiosk_name", "Unknown")
...
def download_csv_timestamps():
    ...
    files = glob.glob(os.path.join(DATA_DIR, "daily_user_count_timestamps_*.json"))
