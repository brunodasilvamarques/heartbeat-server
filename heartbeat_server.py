...
def heartbeat():
    ...
    if kiosk_id:
        kiosks[kiosk_id] = {
            ...
            "last_restricted_user": data.get("last_restricted_user", "None"),
            "restricted_list": sorted(data.get("restricted_list", []))  # ✅ Sorted alphabetically
        }
...
def dashboard():
    ...
    for country, group in sorted(grouped.items()):
        ...
        for kiosk_id, data in sorted(group):
            ...
            restricted_list = "<br>".join(data.get("restricted_list", [])) or "None"
