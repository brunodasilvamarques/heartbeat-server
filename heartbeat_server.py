...
def fetch_jsons_from_kiosks():
    for kiosk_id, info in kiosks.items():
        try:
            ip = info.get("ip_address", kiosk_id)  # fallback to ID if IP is missing
            base_url = f"http://{ip}:5050"
            json_list = requests.get(f"{base_url}/list_jsons", timeout=5).json()

            for fname in json_list:
                save_path = os.path.join("data", fname)
                if not os.path.exists(save_path):  # avoid duplicate
                    r = requests.get(f"{base_url}/download_json/{fname}", timeout=5)
                    if r.status_code == 200:
                        with open(save_path, "wb") as f:
                            f.write(r.content)
                        print(f"📥 Pulled JSON: {fname} from kiosk: {kiosk_id}")
        except Exception as e:
            print(f"❌ Failed to pull from {kiosk_id}: {e}")
...
def send_weekly_csv():
    try:
        ...
        for file in counts_files:
            ...
            print(f"✅ Found file: {file} with internal keys: {list(data.keys())}")  # ← ADD THIS LINE
                
            for date_key, kiosks in data.items():
                if not isinstance(kiosks, dict):
                    continue
                # skip files with no kiosk IDs
                if any(key in ["general", "restricted"] for key in kiosks.keys()):
                    print(f"⚠️ Skipping {file}: missing kiosk ID layer")
                    continue
