...
def download_csv_face():
    ...
    try:
            ...
            for date_key, kiosks_on_date in data.items():
                ...
                for kiosk_id, counts in kiosks_on_date.items():
                    if kiosk_id not in combined_counts[date_key]:
                        combined_counts[date_key][kiosk_id] = counts
        ...
        with open(output_csv, "w", newline="") as csvfile:
            ...
            for date_key, kiosks_on_date in combined_counts.items():
                for kiosk_id, counts in kiosks_on_date.items():
    ...
    try:
        with open(output_csv, "w", newline="") as csvfile:
            ...
            for file in timestamp_files:
                ...
                for date_key, category_data in data.items():
                    for type_name in ["general", "restricted", "left", "right"]:
                        for entry in category_data.get(type_name, []):
