    ...
    for country, group in sorted(grouped.items()):
        ...
        <table>
            <tr>
                ...
                <th>Average General Users</th> <!-- ✅ Correct here -->
                <th>Address</th>
        ...
        for kiosk_id, data in sorted(group):
            ...
            camera_status = data.get("camera_status", "Not Connected ❌") # Keep previous camera status but mark offline in "status" column only
            ...
            # ✅ Just use master_data already loaded
            total_general = 0
            days_recorded = 0
            ...
            average_general = round(average_general, 1)
            ...
            <tr>
                ...
                <td>{address}</td>
                <td>{data.get('software_version', 'Unknown')}</td>
                <td>
                    {"<form method='post' action='/delete/{}' onsubmit='return verifyDelete(\"{}\")'><button type='submit'>Delete</button></form>".format(kiosk_id, kiosk_id) if request.is_admin else "N/A"}
                </td>
            </tr>
            """
        html_rows += "</table><br>"
...
def download_csv():
    ...
    try:
        ...
        with open(temp_csv, "w", newline="") as csvfile:
            fieldnames = ["Date", "Kiosk Code", "Kiosk Name", "General Users"]
