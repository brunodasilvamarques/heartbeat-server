        ...
        for kiosk_id, data in sorted(group):
            ...
            if os.path.exists(kiosk_master_file):
                ...
                for date_key, date_data in kiosk_master.items():
                    ...
                    # ✅ Count only footfall left + right
                    daily_left = len(date_data.get("footfall", {}).get("left", []))
                    daily_right = len(date_data.get("footfall", {}).get("right", []))
                    
                    # Total exits for the day
                    daily_total = daily_left + daily_right
                    
                    # Add to cumulative total
                    total_footfall += daily_total
                    days_recorded += 1

            # ✅ Average footfall per day (only left+right exits)- If there’s only today’s data, include it anyway
            if days_recorded == 0:
                # Try to include today's current data
