...
def dashboard():
    ...
    # ✅ Load master_user_counts.json once
    master_file = "master_user_counts.json"
    master_data = {}
    try:
        if os.path.exists(master_file):
    ...
    for country, group in sorted(grouped.items()):
        ...
        for kiosk_id, data in sorted(group):
            ...
            # If offline, set camera status to Unknown ⚠️
            if delta >= 300:
                camera_status = "Unknown ⚠️"
            else:
                camera_status = data.get("camera_status", "Not Connected ❌")
            ...
            # ✅ Calculate average general users
            total_general = 0
            days_recorded = 0
            for date_entry, kiosks_on_date in master_data.items():
                if kiosk_id in kiosks_on_date:
                    total_general += kiosks_on_date[kiosk_id].get("footfall", 0)
                    days_recorded += 1

            average_footfall = total_general / days_recorded if days_recorded > 0 else 0
            ...
            <tr>
                <td>{kiosk_id}</td>
                <td>{kiosk_name}</td>
                <td><span class="utc-time" data-time="{last_seen}">Loading...</span></td>
                <td>{status}</td>
                <td>{camera_status}</td>
                <td>{restricted_time}</td>
                <td>{restricted_name}</td>
                <td>{restricted_list}</td>
                <td>{average_footfall}</td>
                <td>{address}</td>
            ...
            """.format(
                ...
                actions=(
                    '<form method="post" action="/delete/{}" onsubmit="return verifyDelete(\'{}\')">'
    ...
    <head>
        ...
        <style>
            ...
            button:hover {
                background-color: #1A4ED8; /* darker blue */
                cursor: pointer;
            }
...
def download_csv():
    ...
    try:
        ...
        with open(temp_csv, "w", newline="") as csvfile:
            fieldnames = ["Date", "Kiosk Code", "Kiosk Name", "Country", "Footfall", "Footfall Left", "Footfall Right", "Restricted Users"]
...
def download_csv_face():
    ...
    try:
        ...
        with open(temp_csv, "w", newline="") as csvfile:
            ...
            writer.writerow(["Date", "Kiosk Code", "Kiosk Name", "General Users", "Restricted Users", "Restricted User", "First Seen", "Last Seen"])

            # Load Kiosk Info from metadata
            try:
                with open("kiosk_metadata.json", "r") as f:
                    meta = json.load(f)
                kiosk_code = meta.get("KioskCode", "Unknown Code")
                kiosk_name = meta.get("KioskName", "Unknown Kiosk")
            except:
                kiosk_code = "Unknown Code"
                kiosk_name = "Unknown Kiosk"
            ...
            for day, counts in data.items():
                ...
                if not restricted:
                    writer.writerow([day, kiosk_code, kiosk_name, general, restricted_total, "", "", ""])
                else:
...
def send_weekly_csv():
    try:
        ...
        with open(temp_csv, "w", newline="") as csvfile:
            fieldnames = ["Date", "Kiosk Code", "Kiosk Name", "Country", "Footfall", "Restricted Users"]
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()
