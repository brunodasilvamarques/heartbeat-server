...
def download_csv_face():
    ...
    temp_csv = f"Changebox_Kiosk_FaceCounts_ALL_DATES.csv"
...
def send_daily_csv():
    try:
        ...
        with open(face_csv, "w", newline="") as csvfile:
            ...
            for day, kiosks_on_date in combined_counts.items():
                for kiosk_id, counts in kiosks_on_date.items():
                    ...
                    country = counts.get("country", "Unknown")
                    if not restricted:
                        writer.writerow([day, country, kiosk_id, kiosk_name, general, restricted_total, "", "", ""])
                    else:
                        for user, times in restricted.items():
                            writer.writerow([day, country, kiosk_id, kiosk_name, general, restricted_total, user, times.get("first_seen", ""), times.get("last_seen", "")])
        ...
        email_data = {
            "message": {
                "subject": "📊 Daily ChangeBox CSV Reports",
                "body": {
                    "contentType": "HTML",
                    "content": "Attached are the daily CSV reports for Footfall, Face Detection, and Timestamps."
                },
...
def schedule_daily_email():
    schedule.every(10).minutes.do(send_daily_csv)
...
threading.Thread(target=schedule_daily_email, daemon=True).start()
