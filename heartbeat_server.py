...
def dashboard():
    ...
    for country, group in sorted(grouped.items()):
        for kiosk_id, data in sorted(group):
            ...
            camera_status = (
                ...
                if delta < OFFLINE_THRESHOLD_SECONDS else "Unknown ⚠️"
            )
            ...
            if matching_files:
                ...
                # 📅 Average over the last 7 calendar days (inclusive, today back to today-6)
                today_london = datetime.now(ZoneInfo("Europe/London")).date()
                window_start = today_london - timedelta(days=6)

                for date_key, date_data in kiosk_master.items():
                    # skip root metadata keys
                    if date_key in ["kiosk_code", "kiosk_name", "country", "address"]:
                        continue

                    # parse 'YYYY-MM-DD' daily keys and keep only last 7 days
                    try:
                        day = datetime.strptime(date_key, "%Y-%m-%d").date()
                    except ValueError:
                        continue  # ignore anything unexpected

                    if not (window_start <= day <= today_london):
                        continue
    ...
    {% for row in table_rows %}
        ...
        <tr>
            ...
            <td>
              <div>{{ row.average_footfall }}</div>
              <small style="color:#aaa">Last 7 Days</small>
            </td>
            <td>{{ row.address }}</td>
            <td>{{ row.version }}</td>
            {% if request.is_admin %}
    ...
    <head>
        ...
        <style>
            body {
                ...
                /* OPTIONAL if you don't use .container:
                   padding: 0 16px;  /* adds small left/right inset site-wide */
                */
            }

            /* NEW: wrap page content to keep it in from the edges */
            .container {
                max-width: 1400px;  /* prevents full-bleed on ultra-wide screens */
...
def download_combined_csv():
    ...
    for kiosk_file in glob.glob(os.path.join(DATA_DIR, "*_master_data*.json")):
        ...
        address = kiosk_data.get("address") or "Unknown"

        for date_key, date_data in kiosk_data.items():
            # 👈 skip the non-date metadata keys (now including address)
            if date_key in ["kiosk_code", "kiosk_name", "country", "address"]:
                continue
            ...
            for entry in general_users_list:
                ...
                all_rows.append([
                    date_key, country, kiosk_code, kiosk_name, address,
                    "General User", "-", time_seen, "-", 1
                ])
            ...
            for entry in foot_right_list:
                ...
                all_rows.append([
                    date_key, country, kiosk_code, kiosk_name, address,
                    "Footfall Right", "-", time_seen, "-", 1
                ])

    # Write combined CSV (Address is after Kiosk Name)
    with open(combined_csv, "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow([
            "Date", "Country", "Kiosk Code", "Kiosk Name", "Address",
            "Type", "User/Detail", "First Seen", "Last Seen", "Count"
        ])
        writer.writerows(all_rows)

    # Email it right after generating
...
def send_weekly_csv():
    ...
    # Write CSV (Address after Kiosk Name)
    with open(combined_csv, "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow([
            "Date", "Country", "Kiosk Code", "Kiosk Name", "Address",
...
def _schedule_london_weekly_email():
    """
    ...
    print(f"⏰ Face-Rec weekly CSV scheduled for {run_time.strftime('%Y-%m-%d %H:%M %Z')}"
          f" (in {int(delay)}s)")
    Timer(delay, _weekly_email_task_wrapper).start()

def _weekly_email_task_wrapper():
    send_weekly_csv()          # existing function that builds & emails the CSV
    _schedule_london_weekly_email()  # schedule the next one immediately
...
def start_background_jobs_once():
    ...
    _schedule_london_weekly_email()  # schedules via Timer (DST-aware)
