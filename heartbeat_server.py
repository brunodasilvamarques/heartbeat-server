...
def download_csv():
    ...
    temp_csv = "daily_user_counts_Combined_ALL_DATES.csv"
...
def download_csv_face():
    ...
    temp_csv = "daily_face_detection_Combined_ALL_DATES.csv"
...
def send_weekly_csv():
    try:
        ...
        # --- CSV 1: Footfall ---
        master_file = "master_user_counts.json"
        footfall_csv = f"daily_user_counts_Combined_ALL_DATES.csv"
            ...
            with open(file, "r") as f:
                ...
                for date_key, kiosks in data.items():
                    if date_key not in combined_counts:
                        combined_counts[date_key] = {}
                    for kiosk_id, kiosk_data in kiosks.items():
                        if kiosk_id not in combined_counts[date_key]:
                            combined_counts[date_key][kiosk_id] = {}
                        combined_counts[date_key][kiosk_id].update(kiosk_data)

        face_csv = f"daily_face_detection_Combined_ALL_DATES.csv"
        ...
        for file in timestamp_files:
            ...
            with open(file, "r") as f:
                data = json.load(f)
                kiosks = data.get(file_date, {})  # Focus only on this file’s date
                    if date_key not in combined_counts:
                        combined_counts[date_key] = {}
                    for kiosk_id, kiosk_data in kiosks.items():
                        if kiosk_id not in combined_counts[date_key]:
        ...
        timestamp_csv = f"daily_user_count_timestamps_Combined_ALL_DATES.csv"
...
def download_csv_timestamps():
    ...
    for file in timestamp_files:
        with open(file, "r") as f:
            data = json.load(f)
            for date_key, type_blocks in data.items():
                if date_key not in combined_timestamps:
                    combined_timestamps[date_key] = {}
                for type_name in ["general", "restricted", "left", "right"]:
                    if type_name not in combined_timestamps[date_key]:
                        combined_timestamps[date_key][type_name] = []
                    combined_timestamps[date_key][type_name].extend(type_blocks.get(type_name, []))
