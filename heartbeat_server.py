    ...
    for country, group in sorted(grouped.items()):
        ...
        for kiosk_id, data in sorted(group):
            ...
            """.format(
                ...
                actions=(
                    "<form method='post' action='/delete/{}' onsubmit='return verifyDelete(\"{}\")'><button type='submit' style="background-color:#2D6AFF; color:white; border:none; padding:6px 12px; border-radius:4px;">Delete</button></form>".format(kiosk_id, kiosk_id)
                    if request.is_admin else "N/A"
                )
            )
            ...
            for date_entry, kiosks_on_date in master_data.items():
                for kiosk_id, info in kiosks_on_date.items():
                    writer.writerow({
                        ...
                        "Country": info.get("country", "Unknown"),  # <-- added here
...
def download_csv_face():
    ...
    try:
        ...
        with open(temp_csv, "w", newline="") as csvfile:
            ...
            writer.writerow(["Date", "General User Count", "Restricted User", "First Seen", "Last Seen"])

            for day, counts in data.items():
                general = counts.get("general", 0)
                restricted = counts.get("restricted", {})
                if not restricted:
                    writer.writerow([day, general, "", "", ""])
                else:
                    for user, times in restricted.items():
                        writer.writerow([day, general, user, times["first_seen"], times["last_seen"]])
