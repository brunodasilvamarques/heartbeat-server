...
@app.route("/download_csv_footfall", methods=["GET"])
def download_csv_footfall():
    output = BytesIO()
    writer = csv.writer(output)
    writer.writerow(["Date", "Country", "Kiosk Code", "Kiosk Name", "Footfall", "Footfall Left", "Footfall Right"])

    for file in sorted(glob.glob("data/daily_footfall_counts_*.json")):
        with open(file, "r") as f:
            data = json.load(f)
            for date_key, kiosks_on_date in data.items():
                for kiosk_id, info in kiosks_on_date.items():
                    writer.writerow([
                        date_key,
                        info.get("country", "Unknown"),
                        kiosk_id,
                        info.get("kiosk_name", "Unknown"),
                        info.get("footfall", {}).get("total", 0) if isinstance(info.get("footfall"), dict) else info.get("footfall", 0),
                        info.get("footfall_left", 0),
                        info.get("footfall_right", 0)
                    ])
    output.seek(0)
    return send_file(output, mimetype="text/csv", as_attachment=True, download_name="daily_footfall_counts_Combined_ALL_DATES.csv")
        
@app.route("/download_csv_face", methods=["GET"])
def download_csv_face():
    ...
    for file in sorted(glob.glob("data/daily_user_counts_*.json")):
        with open(file, "r") as f:
            data = json.load(f)
            for date_key, kiosks_on_date in data.items():
    ...
    output.seek(0)
    return send_file(output, mimetype="text/csv", as_attachment=True, download_name="daily_user_counts_Combined_ALL_DATES.csv")

        
@app.route("/download_csv_timestamps", methods=["GET"])
def download_csv_timestamps():
    output = BytesIO()
    writer = csv.DictWriter(output, fieldnames=["Date", "Country", "Kiosk Code", "Kiosk Name", "Type", "Time"])
    writer.writeheader()

    for file in sorted(glob.glob("data/daily_user_count_timestamps_*.json")):
        with open(file, "r") as f:
            data = json.load(f)
    ...
    output.seek(0)
    return send_file(output, mimetype="text/csv", as_attachment=True, download_name="daily_user_count_timestamps_Combined_ALL_DATES.csv")
    ...
    try:
        ...
        for file in counts_files:
            with open(file, "r") as f:
                ...
                for date_key, kiosks in data.items():
                    ...
                    for kiosk_id, kiosk_data in kiosks.items():
                        combined_counts[date_key][kiosk_id] = kiosk_data
        ...
        with open(timestamp_csv, "w", newline="") as csvfile:
            ...
            for file in timestamp_files:
                file_date = os.path.basename(file).replace("daily_user_count_timestamps_", "").replace(".json", "")
                with open(file, "r") as f:
                    data = json.load(f)

                for type_name in ["general", "restricted", "left", "right"]:
                    for entry in data.get(file_date, {}).get(type_name, []):
                        if not isinstance(entry, dict):
                            continue
                        writer.writerow({
                            "Date": file_date,
